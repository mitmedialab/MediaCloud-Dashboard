...
def _sentence_numfound(api_key, keywords, media, start, end):
    ...
    response = cached_split_sentence_count(api_key, query, start, end)
...
def demo_sentence_numfound(keywords):
    ...
    try:
        return _sentence_numfound(app_mc_key, keywords, media, start, end)
    except mcerror.MCException as exception:
        app.core.logger.error("Query failed: "+str(exception))
        content = json.dumps({'error':str(exception)}, separators=(',',':'))
        status_code = exception.status_code
...
def _cached_story_public_docs(api_key, keywords, media, start, end, count=10):
    query = app.core.util.solr_query(keywords, media, start, end)
    app.core.logger.debug("query: _story_docs %s" % query)
    stories = cached_story_public_list(api_key, q=query, rows=count)
    story_count = cached_story_count(api_key, query)
    smaller_stories = [ {'media_url': s['media_url'], 'media_name': s['media_name'],
        'publish_date':s['publish_date'], 'url':s['url'], 'title':s['title']} for s in stories]
    results = {
        'total': story_count['count'],
        'stories': smaller_stories
    }
    return json.dumps(results, separators=(',',':'))
...
def demo_story_docs(keywords):
    media, start, end = demo_params()
    try:
        return _cached_story_public_docs(app_mc_key, keywords, media, start, end)
    except mcerror.MCException as exception:
        app.core.logger.error("Query failed: "+str(exception))
        content = json.dumps({'error':str(exception)}, separators=(',',':'))
        status_code = exception.status_code
...
def _geotagcount(api_key, keywords, media, start, end):
    query = app.core.util.solr_query(keywords, media, start, end)
    app.core.logger.debug("query: _wordcount: %s" % query)
    res = cached_sentence_field_count(api_key, query, tag_sets_id=1011, sample_size=GEO_SAMPLE_SIZE)
...
def demo_geotagcount(keywords):
    media, start, end = demo_params()
    return _geotagcount_handler(app_mc_key, keywords, media, start, end)

def geotagcount_handler_csv(api_key, keywords, media, start, end):
    try:
        results = _geotagcount(api_key, keywords, media, start, end)
...
def demo_geotagcount_csv(keywords):
    media, start, end = demo_params()
    return geotagcount_handler_csv(app_mc_key, keywords, media, start, end)
